...
**What to build:**
- [ ] Full event branding (name, date, venue, tagline) per `DESIGN.md`
- [ ] Eye-zoom entrance animation (first-visit only — see `DESIGN.md` §5)
- [ ] Neuron-network ambient background in corners
- [ ] CTA to login
...
**What to build:**
- [ ] ID + password fields (admin-issued credentials, no self-registration)
- [ ] Error state for invalid credentials
- [ ] Post-login lighter eye-zoom transition into dashboard (`DESIGN.md` §5)
...
**What to build:**
- [ ] Current round/phase (from state machine), current score, quick status summary
- [ ] Links to team profile and live leaderboard
...
**What to build:**
- [ ] Team name, members, avatar/logo if the API provides one
...
**What to build:**
- [ ] Build once in `/shared/components`, must accept a target end-time from the API and count
...
**What to build:**
- [ ] Build in `/shared/components` — Anjishth and both admin devs reuse this
- [ ] Real-time rank + points via polling
- [ ] Up/down movement indicators since last poll (`DESIGN.md` §6)
- [ ] Current team's row visually distinguished
...
**What to build:**
- [ ] Shows current phase name + what's next, using the state machine
...
**What to build:**
- [ ] Task brief display, submission mechanism, confirmation of points awarded
...
**What to build:**
- [ ] Competition mechanic UI + captain reveal moment (use bracket-motif framing per `DESIGN.md` §4)
- [ ] Reflects captaincy advantage status once assigned (exact mechanic TBD — see `PRD.md` §8 open
...
- [ ] Full responsive pass on all screens owned above (mobile-first, per `DESIGN.md` §7)
- [ ] Verify Timer component resyncs correctly across a real network delay/pause scenario
- [ ] Verify leaderboard component performs fine at expected team-count scale (ask organizers
      for expected registration numbers)
- [ ] Loading/error/empty states double-checked on every screen above
